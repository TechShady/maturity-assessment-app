...
import { dynatraceMaturityCategories } from "../maturityModel";
import {
...
export const DtHistory = () => {
  ...
  const loadHistory = useCallback(async () => {
    setLoading(true);
    try {
      const dtRecords = await getDtMaturityHistory();
      setHistory(dtRecords.sort(
  ...
  const handleDelete = async (e: React.MouseEvent, id: string) => {
    ...
    setDeleting(id);
    try {
      await deleteDtMaturityResult(id);
  ...
  if (loading) {
    return (
      <div className="history-container" style={{ textAlign: "center", paddingTop: 80 }}>
        <h2>Loading DT Maturity History...</h2>
      </div>
    );
  }
  ...
  return (
    <div className="history-container">
      ...
      <div className="history-header">
        <h1>DT Maturity History</h1>
        <p>Track your Dynatrace maturity assessment progress over time.</p>
      </div>
      ...
      {sortedHistory.length === 0 ? (
        <div className="history-empty">
          <p>
            {history.length === 0
              ? "No DT maturity assessments completed yet."
